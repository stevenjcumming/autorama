...
save-state.sh (PostToolUse) → Writes current.json with task status
    ↓
on-agent-complete.sh (SubagentStop) → Auto-commits, signals handoff needed
    ↓
Execute command spawns handoff-writer
    ↓
handoff-writer → Writes handoff.md
    ↓
...
`handoff.md` is generated by the `handoff-writer` agent, which is spawned by the `execute.md` command after each task completes. The `on-agent-complete.sh` hook emits a `<handoff-needed>` tag that signals the execute command to spawn the handoff-writer agent with the relevant task context.
...
2. **Signal handoff needed** -- emits `<handoff-needed>` tag for the execute command to spawn the handoff-writer agent
...
| `<handoff-needed>` | Handoff generation requested (execute command spawns handoff-writer) |
