...
1. **Runs review script**: `review.sh` gathers summary data from git and artifacts and writes a `SUMMARY.md` file directly (changes summary, artifact counts, a high-risk scan, deferred issues, and review hints); it prints only a short pointer line, no ANSI output (`generate-report.sh` was merged into `review.sh` in 2.1.0 and no longer exists as a separate script)
...
```
/autocode:review <identifier>
├── Run review.sh (gather data and write SUMMARY.md)
│   ├── Summarize git changes
│   ├── Count artifacts by type
│   ├── Find high-risk items
│   ├── Extract review hints
│   └── Write SUMMARY.md (artifact counts, deferred debt items, review hints list)
...
│       └── scripts/
│           └── review.sh          # Gathers data and writes SUMMARY.md directly

.specs/
└── <identifier>/
    ├── SUMMARY.md          # Generated by review.sh
