...
echo "=== Autocontext Initialization ==="
echo ""
...
# Merge AUTOCONTEXT_PLUGIN_ROOT into settings.local.json
...
merge_env_setting() {
  ...
  if command -v jq &> /dev/null; then
    if [ -f "$file" ]; then
      ...
      tmp="$(mktemp)"
      jq --arg dir "$plugin_dir" '.env["AUTOCONTEXT_PLUGIN_ROOT"] = $dir' "$file" > "$tmp"
      mv "$tmp" "$file"
    else
      tmp="$(mktemp)"
      jq -n --arg dir "$plugin_dir" '{"env": {"AUTOCONTEXT_PLUGIN_ROOT": $dir}}' > "$tmp"
      mv "$tmp" "$file"
    fi
...
if os.path.exists(file_path):
    try:
        ...
        sys.exit(1)
data.setdefault('env', {})['AUTOCONTEXT_PLUGIN_ROOT'] = plugin_dir
...
" "$file" "$plugin_dir"
    ...
    echo "  {\"env\": {\"AUTOCONTEXT_PLUGIN_ROOT\": \"$plugin_dir\"}}"
    return 1
  fi
}

if merge_env_setting "$SETTINGS_FILE" "$PLUGIN_DIR"; then
  echo "Set AUTOCONTEXT_PLUGIN_ROOT=$PLUGIN_DIR"
...
echo "  2. Run /autocontext:create <path> to generate your first CONTEXT.md"
echo ""
